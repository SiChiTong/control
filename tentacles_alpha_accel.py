...
class traject_calc:
  ...
  def build_trace(self, car, pos, plan, element):
    ...
    if time_len>1:
      ...
      for t in plan.time_list[1:]:
        ...
        alpha[i+1] = alpha[i]+plan.omiga0*plan.dt
        # 不超限值
        if np.abs(alpha[i+1])>np.abs(car.max_omiga) or np.abs(alpha[i+1])>(np.abs(soft_alpha)):
          alpha[i+1] = alpha[i]
        # 偏角导致航向变化
        ...
        if v[i+1]>element.speed_limit:
          ...
          v[i+1] = 0
  #      total_s += v[i]*plan.dt
        total_s += np.abs(x[i+1]-x[i]) # 最有效的应该是计算道路路径长度，而不是车辆行程长度，这里简单用x代替
        i += 1
    # 输出结果
    result.x = x
    result.y = y
  ...
  def rand_build_tentacle(self, trace_num, car, pos0, element, end_time, curve):
    ...
    for i in range(trace_num):
      ...
      time_list[i] = 0.5*(i+1) # const
      alpha_list[i] = car.max_alpha # const
      # 将转角速度分辨率设为1/100 rad/s
      omiga_list[i] = np.random.randint(-100,101)*car.max_omiga/100.0*curve      
      # 将加速度分辨率设为？
